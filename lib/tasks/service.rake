namespace :service do
  ...
  task :daemons, [:command] do |_task, args|
    @daemons = %w[rango blockchain cron_job upstream deposit trade_executor deposit_coin_address withdraw_coin influx_writer account_sidekiq order_processor matching]
  ...
  task :app, [:command] => [:setup] do |task, args|
    args.with_defaults(:command => 'start')
  ...
  task :frontend, [:command] do |task, args|
    ...
    def start
      ...
      sh 'docker-compose up -d sonic mobile'
    end

    def stop
      puts '----- Stopping the frontend -----'
      sh 'docker-compose rm -fs sonic mobile'
    end
  ...
  desc 'Set up daemon container'
  task :daemon, [:command] => ['render:config'] do |task, args|
    args.with_defaults(:command => 'start')
  ...
  desc 'Set up user container'
  task :user, [:command] => ['render:config'] do |task, args|
    args.with_defaults(:command => 'start')
  ...
  desc 'Set up database container'
  task :db, [:command] => ['render:config'] do |task, args|
    args.with_defaults(:command => 'start')
